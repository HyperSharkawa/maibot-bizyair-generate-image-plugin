...
- **App 预设命令**：通过聊天命令 `/dr list` 查看所有可用预设，`/dr use <预设名>` 在运行时切换当前激活的预设，`/dr switch <on|off>` 在运行时动态开关生图功能，无需重启。
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `action_enabled` | `bool` | `true` | 生图功能总开关。关闭后会直接拒绝所有生图请求，并发送“当前图片生成功能未开启”。 |
...
- `permission_control.command_user_list` / `command_user_list_mode`：控制 `/dr list`、`/dr use` 和 `/dr switch` 命令的使用权限。
...
在运行时动态开关图片生成功能，并将新的 `action_enabled` 值写回 `config.toml`
